...
module tb_uart_top;
		...
		begin
			...
			begin : wait_loop
				for (i = 0; i < timeout_cycles; i = i + 1) begin
					#1ps; /* observe rx_ready after NBA updates */
					if (rx_ready) begin
						ready_seen = 1'b1;
	...
	task automatic rx_send_and_expect(input [7:0] data_byte, input stop_ok);
		...
		begin
			...
			/* rx_ready is asserted mid-stop-bit, so start waiting immediately. */
			wait_rx_ready(40 * CYCLES_PER_BIT_APPROX, rx_data_seen, rx_frame_error_seen);
			...
			/* rx_ready should be a single-cycle pulse at uart_top level */
			@(posedge clk);
			if (rx_ready)
				fail("rx_ready was not a pulse");
